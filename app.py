...
def clean_agent_output(output) -> str:
    ...
    If it is a list of blocks/dicts containing metadata and signatures (common in Gemini), 
    it extracts and joins only the readable text segments.
    """
...
if st.session_state.parsed_resume is None:
    # If no data exists, display a landing welcoming dashboard
    ...
    with tab2:
        ...
        st.write("These jobs were extracted from your database using vector similarity and graded by Gemini.")
        
        if st.session_state.matching_results:
            for idx, match in enumerate(st.session_state.matching_results.matches):
                with st.expander(f"📌 {match.title} | Match Score: {match.match_score}%", expanded=(idx==0)):
                    col_score, col_details = st.columns([1, 4])
                    
                    with col_score:
                        st.metric(label="Match Alignment", value=f"{match.match_score}%")
                        ...
                        if st.button("🎯 Select This Role", key=f"sel_role_{match.jd_id}"):
                            ...
                            with st.spinner("Generating detailed skill gap report and roadmap..."):
                                try:
                                    ...
                                    # Initialize progress checklists and text responses
                                    st.session_state.completed_weeks = {w.week_number: False for w in roadmap.weeks}
                                    st.session_state.completed_questions = {q.question_id: False for q in interview_set.questions}
                                    st.session_state.mock_answers = {}
                                    st.session_state.mock_feedback = {}
        ...
        if not st.session_state.selected_job or not st.session_state.interview_set:
            ...
            for q in st.session_state.interview_set.questions:
                with st.container(border=True):
                    ...
                    # 1. Text Area for user to type their answer
                    user_draft = st.text_area(
                        "Type your response to practice:",
                        value=st.session_state.mock_answers.get(q.question_id, ""),
                    ...
                    st.session_state.mock_answers[q.question_id] = user_draft
                            ...
                            if not user_draft.strip():
                                ...
                                with st.spinner("AI Panel is reviewing your response..."):
                                    try:
                                        ...
                                        st.session_state.mock_feedback[q.question_id] = feedback
                    ...
                    if q.question_id in st.session_state.mock_feedback:
                        st.divider()
                        st.markdown("### 🤖 AI Panel Evaluation Feedback")
                        st.info(st.session_state.mock_feedback[q.question_id])
    ...
    with tab5:
        ...
        if not st.session_state.selected_job or not st.session_state.roadmap or not st.session_state.interview_set:
            ...
            with col_weeks:
                ...
                for week in st.session_state.roadmap.weeks:
                    st.session_state.completed_weeks[week.week_number] = st.checkbox(
                        f"Complete Week {week.week_number}: {week.theme}",
                        value=st.session_state.completed_weeks.get(week.week_number, False),
            ...
            with col_qs:
                ...
                for q in st.session_state.interview_set.questions:
                    st.session_state.completed_questions[q.question_id] = st.checkbox(
                        f"Answered Q{q.question_id}: {q.category}",
                        value=st.session_state.completed_questions.get(q.question_id, False),
